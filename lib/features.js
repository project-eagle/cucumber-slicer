...
function getFeatureTop(parsed, i = 0, mapOfGeneratedFeatureTags = {}, z = 0) {
  ...
  let output = '';
  let featureNameTags = featureNameAsTags(featureTitle, 1);
  ...
  if (featureLevelTags.length) {
    output += extractTags(featureLevelTags) + ` @ScenarioModulo${z} @FeatureSplitNum${i} ` + (featureNameTags.join(" ")) + " \n";
  }
  else {
...
function featureNameAsTags(name, maxWords = 3) {
  ...
  for (let i = 1; i <= maxWords && i <= words.length; i++) {
    // Concatenate the first 'i' words to form a tag
    const tag = words.slice(0, i).join('_').replace(/\W+/g, '');
